...
export const agendamentoService = {
  create: async ({
    ...
    for (
      let i = dateDataInicio.getTime();
      i < dateDataFim.getTime();
      i = dateDataInicio.setDate(dateDataInicio.getDate() + frequencia * 7)
    ) {
      ...
      VALUES (
        ...
        '${dateDataInicio
          ...
          .slice(
            0,
            10
          )}', '${dataFim}', '${descricao}', ${frequencia}, ${idprospect}, ${idsituacao})
      `);
    }
  ...
  showByDate: async (date: Date, id: number) => {
    const dateToFind = date.toISOString().slice(0, 10);
    const agendamentos = await db.query(`
    SELECT
    idagendamento,
    data_visita as dataVisita,
    ...
    ON agendamento.prospect_idprospect = prospect.idprospect
    INNER JOIN endereco ON  prospect.endereco_idendereco = endereco.idendereco
    WHERE data_visita= '${dateToFind}' AND user_iduser = ${id}
    `);
  ...
  delete: async (idagendamento: number) => {
    await db.query(
      `DELETE FROM agendamento WHERE idagendamento=${idagendamento}`
    );
  },

  deleteAll: async (id: number, codAgendamento: string) => {
    await db.query(
      `DELETE FROM agendamento WHERE idagendamento > ${id} AND cod_agendamento='${codAgendamento}'`
    );
  },
};
